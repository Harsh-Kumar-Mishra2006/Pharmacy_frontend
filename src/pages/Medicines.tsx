...
import { type AvailableMedicine } from "../types";

// Augmented type returned by /medicines/available
...
const Medicines: React.FC = () => {
  ...
  const [selectedMedicine, setSelectedMedicine] =
    useState<AvailableMedicine | null>(null);
  const [purchaseModal, setPurchaseModal] = useState<AvailableMedicine | null>(
    null,
  );
  ...
  const loadMedicines = async () => {
    clearError();
    const res = await getAvailableMedicines({ limit: 100 });
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortBy) {
      ...
      case "newest":
      default:
        result.sort((a, b) => a.name.localeCompare(b.name)); // already ordered by backend
    }
    return result;
  }, [medicines, searchTerm, categoryFilter, sortBy]);

  const handleBuyClick = (medicine: AvailableMedicine) => {
    if (!isAuthenticated) {
  ...
  const confirmPurchase = () => {
    ...
    if (purchaseQty < 1 || purchaseQty > purchaseModal.total_quantity) {
      setMessage({
        type: "error",
        text: `Quantity must be between 1 and ${purchaseModal.total_quantity}`,
      });
      return;
    }
    navigate("/purchase/checkout", {
      state: { medicine: purchaseModal, quantity: purchaseQty },
    });
  };
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
            <FaCapsules className="text-light-orange" /> Browse Medicines
          </h1>
        ...
        {!isLoading && filtered.length > 0 && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filtered.map((m) => {
              const outOfStock = m.total_quantity === 0;
              const lowStock = m.total_quantity > 0 && m.total_quantity <= 10;
              const displayPrice = m.min_price ?? 0;
              return (
                <div
                  key={m.id}
                    ...
                    <div className="mt-3 flex items-end justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-xs text-gray-500 flex items-center gap-1 justify-end">
                          <FaBoxes className="text-xs" /> {m.total_quantity} in
                          stock
                        </p>
                      </div>
                    </div>
          ...
          >
                  ...
                  <div className="text-right">
                    <p className="text-sm text-gray-600">
                      ...
                      <span className="font-bold">
                        {selectedMedicine.total_quantity}
                      </span>
                    </p>
                  </div>
                </div>
            ...
            <div className="sticky bottom-0 bg-white border-t border-gray-100 p-4 flex gap-3">
              ...
              <button
                ...
                disabled={selectedMedicine.total_quantity === 0}
          ...
          >
            ...
            <div className="mb-4">
              ...
              <input
                ...
                className={`w-full px-4 py-3 rounded-lg border text-lg font-semibold focus:ring-2 ${
                  purchaseQty > purchaseModal.total_quantity || purchaseQty < 1
                    ? "border-red-400 focus:ring-red-400"
                    : "border-gray-300 focus:ring-light-orange"
                }`}
              />
              {purchaseQty > purchaseModal.total_quantity && (
                <p className="text-xs text-red-500 mt-1 flex items-center gap-1">
                  <FaExclamationCircle /> Maximum available is{" "}
                  {purchaseModal.total_quantity}
                </p>
              )}
            </div>
